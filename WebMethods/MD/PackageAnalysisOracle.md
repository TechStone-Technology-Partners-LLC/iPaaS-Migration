...
|---|---|---|---|
Create a new recipe using NewRecipe.json as a reference
| Trigger | callable_recipe | Update Product Info | Receives 2-field JSON |
For Trigger use WorkatoServiceTrigger.json as a reference. For fields use ProductNumber and ItemNo as schema fields.
| 1 | Oracle Select Rows | Select Rows | WHERE condition , 1 params |
Configure the Workato Action Oracle Select Rows. Ask for where condition, table name and output columns.Use OracleSearchRows.json as a reference for step 1
| 2 | IF/ELSE | Check Size=0 | Condition: Size == “0” |
| 2a | STOP With Error  | STOP | Reason for failure is product not found in inventor |
Use Stop.json as a reference for step 2a
| 2b | Return Response for callable_recipe | Return response from Check if product is available in Oracle ERP |
Use WorkatoServiceSendReply.json as reference for step 2b
